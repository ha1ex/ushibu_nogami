...
## Повторный аудит координат · 08.09.2026

Все diagram-слайды 03, 04, 06, 08, 10–13, 15–18 и 21 повторно проверены на 1600×836, 1366×704 и 1280×656. Автоматическая проверка для каждого размера подтверждает: якорь остаётся в рамке радара, лидер начинается в якоре и заканчивается на границе плашки, маршруты не выходят за визуал, плашки не перекрывают друг друга и точки.

Семантические `u/v` заново привязаны к аннотированной координатной базе того же `assets/anubis-radar-csnades-2026.webp`; `tx/ty` и `lu/lv` изменялись отдельно только для композиции. Контрольные якоря в долях изображения: `T Spawn` 0.440/0.932, `CT Spawn` 0.399/0.207, `A Main` 0.809/0.426, `A Connector` 0.602/0.377, `A Heaven` 0.674/0.228, `Water` 0.681/0.494, `Bridge` 0.469/0.596, `Mid Doors` 0.524/0.499, `Drop` 0.719/0.531, `B Main` 0.244/0.570, `B Connector` 0.467/0.327, `E-box` 0.349/0.588, `Back B` 0.359/0.499, `Pillar` 0.264/0.559.

Ниже сохранён исходный аудит 07.09.2026 как история; его старые размеры и пути скриншотов не описывают повторную проверку.
