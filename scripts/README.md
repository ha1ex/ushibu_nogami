...
Запускаются через `corepack pnpm` (см. корневой `package.json`). Зависимости — в `scripts/semantic/`
и `scripts/skillopt/` (поставить: `corepack pnpm run setup`).

| Команда | Файл | Назначение |
|---|---|---|
| `corepack pnpm kb:init` | `kb-init.mjs` | параметризация клона: цель проекта → AGENTS.md/core.md, `--strip-demo`, `--level 0..3` |
| `corepack pnpm kb:index` | `semantic/index.mjs` | построить/обновить гибридный индекс (+ `doc_date`, `status`) |
| `corepack pnpm kb:search` | `semantic/search.mjs` | hybrid-поиск (vector + BM25 + RRF + graph + temporal; stub/deprecated скрыты) |
| `corepack pnpm kb:think` | `semantic/think.mjs` | промпт-синтез с цитатами |
| `corepack pnpm kb:eval` | `semantic/eval.mjs` | retrieval-eval (recall@k/MRR) + регрессия vs baseline + D3-гейт (stub в топах); A/B `--graph/--rerank` |
| `corepack pnpm kb:doctor` | `kb-doctor.mjs` | health-check KB (+ advisory: stale inbox/answer-cards, stub-карточки) |
| `corepack pnpm kb:verify` | `semantic/verify.mjs` | проверка цитат `[source:]` (Tier-1 gate: traversal/coverage + FACT-advisory + critique); `--scan --provenance` — CI-гейт (N3/N4); `--scan` без `--no-semantic` — Tier-2 из индекса (D2) |
| `corepack pnpm kb:metrics` | `kb-metrics.mjs` | тренды качества из журнала по неделям (D1) |
| `corepack pnpm kb:digest` | `kb-digest.mjs` | утренний дайджест состояния KB (D4; см. `../docs/automation.md`) |
| `corepack pnpm kb:update` | `kb-update.mjs` | обновление ядра от upstream по `.template-manifest.json` (C3) |
| — | `kb-critic.mjs` | revision-промпт по битым цитатам; `--execute` гоняет verify→revise через `claude` (N1) |
| `corepack pnpm kb:dream` | `dream-cycle.mjs` | еженедельный LLM-аудит + fact-консолидация (пишет в `.context/`) |
