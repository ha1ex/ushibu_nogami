...
- **Всегда пушим в `main` сразу.** Любое законченное изменение — коммит и немедленный
  `git push origin HEAD:main`. Не копим работу в ветке, не ждём отдельного разрешения на пуш и
  не спрашиваем «пушить?»: разрешение выдано один раз и действует всегда.
- «Законченное изменение» = связная единица работы, которая проходит гейты. Не пушим заведомо
  сломанное состояние — доводим до зелёного и пушим.
- Перед пушем: `pnpm kb:doctor` (EXIT 0) и гейт цитат `scripts/git-hooks/pre-push`
  (включается один раз: `git config core.hooksPath scripts/git-hooks`).
  Красный гейт — чиним, а не обходим `--no-verify`.
- Ветка воркспейса всегда ff-совместима с `main`. Разошлись — `git pull --rebase`, потом пуш.
- Без force-push в `main`.
